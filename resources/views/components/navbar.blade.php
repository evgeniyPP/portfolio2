  ...
  <nav x-data="{ isOpen: false }" class="bg-gray-800 fixed top-0 w-full">
      ...
      <div class="relative flex items-center justify-between h-16">
        <div class="absolute inset-y-0 right-0 flex items-center sm:hidden">
          ...
          <button x-on:click="isOpen = !isOpen" class="inline-flex items-center justify-center p-2 rounded-md text-gray-400 hover:text-white hover:bg-gray-700 focus:outline-none focus:bg-gray-700 focus:text-white transition duration-150 ease-in-out" aria-label="Main menu" aria-expanded="false">
            ...
            <svg x-bind:class="{ 'hidden': isOpen }" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    <div x-bind:class="{ 'hidden': !isOpen }" class="sm:hidden">
      ...
      <div class="px-2 pt-2 pb-3">
        ...
        <a href="#" class="mt-1 block px-3 py-2 rounded-md text-base font-medium text-gray-300 hover:text-white hover:bg-gray-700 focus:outline-none focus:text-white focus:bg-gray-700 transition duration-150 ease-in-out">Скачать резюме (.docx)
        </a>
        <a href="#" class="mt-1 block px-3 py-2 rounded-md text-base font-medium text-gray-300 hover:text-white hover:bg-gray-700 focus:outline-none focus:text-white focus:bg-gray-700 transition duration-150 ease-in-out">Скачать эту страницу (.pdf)
        </a>
      </div>
    </div>
  </nav>
